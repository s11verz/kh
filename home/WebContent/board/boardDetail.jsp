...
<div class="container-900">
	...
	<div class="row text-left" style="border:1px solid gray;">
		<div class="float-container">
			<div class="left"><%=replyDto.getReplyWriter()%></div>
			
			<!-- 
				수정과 삭제 메뉴는 "본인 글"에만 표시되어야 한다
				= 본인글 : 댓글작성자번호(replyDto.getReplyWriter()) == 로그인된 회원번호(memberNo) 
			-->
			<%if(replyDto.getReplyWriter() == memberNo){ %>
			<div class="right">
				<a class="reply-edit-btn">수정</a> 
				| 
				<a class="reply-delete-btn" href="replyDelete.kh?replyNo=<%=replyDto.getReplyNo()%>&replyOrigin=<%=boardNo%>">삭제</a>
			</div>
			<%} %>
		</div>
		<!-- 화면 표시 댓글 -->
		...
		<%} %>
		<div><%=replyDto.getReplyTime().toLocaleString()%></div>
	</div>
	<%} %>
